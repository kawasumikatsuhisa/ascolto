...
| 入力 | 綴りを打って答え合わせ（アクセント記号・大文字小文字は問いません） |
...
入力モードでは、答えが日本語になる問題（`伊 → 日本語` の曜日・月・単語）だけ
選択式に切り替わります。日本語入力に切り替えて打つのは電車では現実的でないためです。
時刻は `14:45` でも `1445` でも通ります。
...
PC で使うときは、選択式なら 1〜4 で選んで Space で次へ。入力は Enter で
答え合わせ、もう一度 Enter で次へ。めくる方式は Space でめくって 1/2/3 で採点。
いずれも S で読み上げ。

自己採点（だめ / あやふや / 言えた）が出るのは「めくって自己採点」のときだけです。
選択式と入力は答え合わせの時点で正誤が決まっているので、そのまま次へ進みます。
